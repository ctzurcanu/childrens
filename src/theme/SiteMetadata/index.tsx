...
// Keep fallback English articles accessible without indexing them as French.
function useDocumentLocales() {
  const {pathname} = useLocation();
  const data = usePluginData('childrens-seo') as {docLocales: Record<string, string[]>};
  return data.docLocales[pathname.replace(/\/$/, '')];
}
...
function AlternateLangHeaders(): ReactNode {
  ...
  const docLocales = useDocumentLocales();
  ...
  return (
    <Head>
      {Object.entries(localeConfigs).filter(([locale]) => !docLocales || docLocales.includes(locale)).map(([locale, {htmlLang}]) => (
        <link
          key={locale}
          rel="alternate"
      ...
      {Object.values(localeConfigs)
        .filter((config) => currentHtmlLang !== config.htmlLang && (!docLocales || docLocales.some(locale => localeConfigs[locale] === config)))
        .map((config) => (
          <meta
...
export default function SiteMetadata(): ReactNode {
  ...
  const docLocales = useDocumentLocales();
  const isFallback = docLocales && !docLocales.includes(currentLocale);
  const {pathname} = useLocation();
  const isNotFound = /\/404\.html\/?$/.test(pathname);
